...
//
// ml_predictive_incidents holds ONLY validation and future rows — there is no
// third "past, unscored" prediction_type. Its earliest date IS the validation
// start; its latest IS the far edge of the forecast horizon. That fact is what
// lets the window-resolution and query logic below stay simple: "future" can
// always be selected by prediction_type alone, with no date arithmetic needed
// to keep it out of a Range-bounded WHERE clause.
...
export function buildIncidentPredictiveResponse(
  ...
  const fullDaily = allDates.map((date) => {
    ...
    return {
      ...
      // ml_predictive_incidents' own rainfall_mm wins where it exists (it's
      // what the model was actually scored against — Open-Meteo for future
      // dates); the hourly_weather-derived figure fills the rest of the
      // window, which the predictions table was never going to cover.
      rainfallMm: pred?.rainfall_mm != null ? Number(pred.rainfall_mm) : (rainByDate.get(date) ?? null),
