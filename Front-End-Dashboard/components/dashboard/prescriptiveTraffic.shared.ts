...
/** Drop the memoised payload so the next loadForecast() refetches.
 *
 *  The cache is module-scoped and never expired, which is right for its
 *  original job — three panels mounting together share one request. It is
 *  wrong for a tab left open: the congestion forecast is republished every
 *  hour by the "SmartFlow congestion refresh" task, and without this the page
 *  would hold the payload it fetched on first mount until a manual reload,
 *  slowly counting down to an empty grid while fresh rows sat in the database.
 */
export function invalidateForecast(): void {
  cache = null;
}
