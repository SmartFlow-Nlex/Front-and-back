...
/**
 * Incident feature-evidence panel — shared by the "Does weather predict
 * incidents?" and "Does traffic volume predict incidents?" panels on the
 * incident predictive tab.
 *
 * Structurally this mirrors WeatherEvidencePanel.tsx on the traffic module
 * (same collapsed pills, same expand/collapse mechanics, same bar list +
 * with/without model comparison) — parameterized here since the incident tab
 * needs two of these rather than one. Both halves are computed live from the
 * warehouse at request time (see getIncidentWeatherEvidenceFromDb /
 * getIncidentVolumeEvidenceFromDb in incident.service.ts), nothing is stored
 * prose or a hardcoded figure.
 *
 * Collapsed by default — it answers a question the reader may not have asked
 * yet, so it should not push the metrics table down the page on every load.
 */
...
type Props = {
  /** Path suffix under /api/incident/, e.g. "weather-evidence" or "volume-evidence". */
  endpoint: string;
  title: string;
  /** Shown only while collapsed, under the title. */
  subtitle: string;
  /** Which correlation `variable` key the parent chart currently overlays, bolded + tagged "on chart". */
  plotted: string;
  /** e.g. "weather" / "volume" — used in the with/without section heading. */
  featureLabel: string;
  /** Free text explaining which models actually take this feature as an input — differs per feature, so not derived here. */
  explainerNote: string;
};

export default function IncidentFeatureEvidencePanel({ endpoint, title, subtitle, plotted, featureLabel, explainerNote }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`${BACKEND}/api/incident/${endpoint}`)
      .then((r) => r.json())
      .then((j) => {
        if (cancelled) return;
  ...
  const multiVariable = corr.length > 1;

  // Verdict is derived, not asserted: does this feature meaningfully help ANY model?
  const helped = models.filter((m) => (m.deltaPts ?? 0) > 0.05).length;
  const verdict =
    models.length === 0 ? null : helped === 0 ? `No model improves with ${featureLabel}` : `Helps ${helped} of ${models.length} models`;
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ minWidth: 210 }}>
          ...
          {!open && <p style={{ margin: "3px 0 0", fontSize: "0.78rem", color: "var(--text-secondary)" }}>{subtitle}</p>}
        </div>
        ...
        {!open && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", flex: 1, justifyContent: "flex-end" }}>
            {strongest && (
              ...
              >
                {multiVariable && <span style={{ color: "var(--text-secondary)" }}>Strongest</span>}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {strongest && multiVariable && (
            <p style={{ margin: 0, fontSize: "0.76rem", color: "var(--text-secondary)", lineHeight: 1.5 }}>
              ...
              {strongest.pearson?.toFixed(3)} — still {strengthOf(strongest.pearson).label}.
            </p>
          )}

          {/* The controlled test: same model, same protocol, feature in vs out */}
          {models.length > 0 && (
            <div style={{ borderTop: "1px solid var(--border-default)", paddingTop: 12 }}>
              <div style={{ fontSize: "0.79rem", fontWeight: 700, color: "var(--text-primary)", marginBottom: 7 }}>
                Same model, trained with and without {featureLabel}
              </div>
                ...
                {models.map((m) => {
                  ...
                  return (
                    <span key={m.model} style={{ fontSize: "0.76rem", color: "var(--text-secondary)" }}>
                      ...
                      {m.withFeature?.toFixed(2)}% vs {m.withoutFeature?.toFixed(2)}%{" "}
                      <b style={{ color: tone }}>
              ...
              <p style={{ margin: "8px 0 0", fontSize: "0.74rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
                Lower WMAPE is better. LSTM and GRU are absent because they are univariate — the method has no
                mechanism for external inputs, so there is no with/without pair to compare.
              </p>
            </div>
          )}
        </div>
