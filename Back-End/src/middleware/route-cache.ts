...
export function routeCache(req: Request, res: Response, next: NextFunction): void {
  ...
  res.json = ((body: unknown) => {
    /* Never store a degraded response.
     *
     * When the database is briefly unreachable the controllers do not fail --
     * they answer 200 with `source: "mock"` and a zero-filled payload, so the
     * dashboard renders something instead of erroring. That is fine for one
     * request and poison for a cache: a single blip put "totalVolume: 0" in
     * here, and for the next ten minutes EVERY reader got the zeros, with
     * stale-while-revalidate handing them out even after the database came
     * back. The card showed an empty corridor while the database was healthy,
     * which is indistinguishable from real data and was repeatedly read as the
     * dashboard being broken.
     *
     * Only a real answer earns a cache slot. A mock still reaches the caller
     * (it is passed straight through), it simply does not outlive its request,
     * so the next one retries the database and self-heals within seconds.
     */
    const degraded =
      typeof body === "object" && body !== null && (body as { source?: unknown }).source === "mock";
    if (res.statusCode === 200 && !degraded) store.set(url, { at: Date.now(), status: 200, body });
    if (degraded) {
      // Keep it out of the browser's cache too, so a blip cannot survive there
      // after the server has forgotten it.
      res.setHeader("Cache-Control", "no-store");
      store.delete(url);
    }
    res.setHeader("X-Cache", degraded ? "BYPASS-DEGRADED" : bypass ? "REFRESH" : "MISS");
