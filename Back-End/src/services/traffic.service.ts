...
// [ML-01] Get Predictive Volume (LSTM) from Database
export async function getMLPredictiveVolume() {
  if (!db) return null;
  try {
    const { rows } = await db.query(`SELECT forecast_date as "date", actual_volume, predicted_volume, is_holdout, is_future FROM gold.ml_predictive_volume ORDER BY forecast_date ASC`);
    return rows;
  } catch (error) {
