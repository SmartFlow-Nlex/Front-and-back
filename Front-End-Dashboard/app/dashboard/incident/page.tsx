...
export default function IncidentPage() {
  ...
  // Model comparison toggle — defaults to the champion once data loads.
  const [selectedModel, setSelectedModel] = useState<string>("");
  useEffect(() => {
    if (predictiveData && !selectedModel) setSelectedModel(predictiveData.summary.modelUsed);
  }, [predictiveData, selectedModel]);
  ...
  // ---------- Model comparison toggle (Predictive tab) ----------
  const modelsByRank = useMemo(
    () => (predictiveData ? [...predictiveData.modelComparison].sort((a, b) => a.rank - b.rank) : []),
    [predictiveData]
  );
  const championEntry = useMemo(() => modelsByRank.find((m) => m.isChampion) ?? null, [modelsByRank]);
  const activeModel = useMemo(
    () => modelsByRank.find((m) => m.model === selectedModel) ?? championEntry,
    [modelsByRank, selectedModel, championEntry]
  );

  // Non-champion models have no exported day-level predictions — approximate their
  // curve by scaling the champion's real series by the candidate's own MAE ratio.
  // Exact per-model metrics (below) are always real; only this curve is illustrative.
  const displayPredictedData = useMemo(() => {
    if (!predictiveData) return [];
    const raw = predictiveData.chartData.predictedData;
    if (!activeModel || activeModel.isChampion || !championEntry || championEntry.MAE === 0) return raw;
    const ratio = activeModel.MAE / championEntry.MAE;
    return raw.map((v) => (v == null ? null : Number((v * ratio).toFixed(2))));
  }, [predictiveData, activeModel, championEntry]);

  // Three-tier status color: champion = winner, overfitting = rejected-style
  // warning, everything else that beat the naive baseline = runner-up.
  type ModelCategory = "winner" | "runnerup" | "overfitting";
  const CATEGORY_COLOR: Record<ModelCategory, string> = {
    winner: "#059669",      // emerald-600
    runnerup: "#f59e0b",    // amber-500
    overfitting: "#e11d48", // rose-600
  };
  const modelCategory = (m: PredictiveData["modelComparison"][number]): ModelCategory => {
    if (m.isChampion) return "winner";
    if (m.diagnosis === "OVERFITTING") return "overfitting";
    return "runnerup";
  };
  const modelStatusText = (m: PredictiveData["modelComparison"][number]) => {
    if (m.isChampion) return `Rank #${m.rank} · Selected`;
    if (m.diagnosis === "OVERFITTING") return `Rank #${m.rank} · Overfitting`;
    return `Rank #${m.rank} · Just Right`;
  };
  const activeColor = activeModel ? CATEGORY_COLOR[modelCategory(activeModel)] : CATEGORY_COLOR.winner;
            ...
            {predictiveLoading ? (
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: "14px", width: "100%" }}>
                  {[
                    { label: `Predicted Incidents (${predictiveData.modelInfo.forecastHorizon})`, value: predictiveData.summary.totalPredictedNext7Days != null ? String(predictiveData.summary.totalPredictedNext7Days) : "Pending", accent: "#3e67ef" },
                    ...
                    { label: "Model Used",                  value: activeModel?.model ?? predictiveData.summary.modelUsed,  accent: "#1e293b" },
                  ].map((c) => (
                ...
                <div style={{ background: "#fff", borderRadius: "12px", padding: "24px 28px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0", boxSizing: "border-box", width: "100%" }}>
                  {/* Chart header — title/pill on the left, model switcher top-right */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px", flexWrap: "wrap", gap: "12px" }}>
                    <div>
                      <h3 style={{ fontSize: "1.05rem", fontWeight: 700, color: "#1e293b", margin: 0, display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                        Incident Volume Walk-Forward Forecast
                        <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: "20px", fontSize: "0.72rem", fontWeight: 700, background: activeColor, color: "#fff" }}>
                          {activeModel?.model ?? predictiveData.summary.modelUsed}
                        </span>
                      </h3>
                      <p style={{ fontSize: "0.82rem", color: "#64748b", margin: "4px 0 0" }}>
                        Toggle models below to visualize their prediction accuracy against the ground truth on NLEX.
                      </p>
                    </div>

                    {/* Model switcher bar — Traffic Volume chart's pill-toolbar chrome (white, bordered, fully rounded), 3-tier status color */}
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", flex: "0 1 auto", minWidth: 0 }}>
                      ...
                      <span style={{ fontSize: "0.74rem", fontWeight: 700, color: "#4b5e7d", letterSpacing: "0.02em", whiteSpace: "nowrap" }}>Models</span>
                      <div style={{ display: "inline-flex", flexWrap: "wrap", gap: "2px", padding: "3px", background: "#fff", border: "1px solid #dce2ef", borderRadius: "999px" }}>
                        {modelsByRank.map((m) => {
                          const isActive = m.model === (activeModel?.model ?? "");
                          const color = CATEGORY_COLOR[modelCategory(m)];
                          return (
                            <button
                              key={m.model}
                              onClick={() => setSelectedModel(m.model)}
                              aria-pressed={isActive}
                              title={`${m.model} (${modelStatusText(m)})`}
                              style={{
                                display: "inline-flex",
                            ...
                            >
                              <span style={{ display: "inline-flex", alignItems: "center", fontSize: "0.78rem", fontWeight: 700, color: isActive ? "#fff" : "#334155" }}>
                                {isActive && (
                              ...
                              <span style={{ fontSize: "0.64rem", fontWeight: 600, color: isActive ? "rgba(255,255,255,0.85)" : "#94a3b8" }}>
                                ({modelStatusText(m)})
                              </span>
                            </button>
                          );
                  ...
                  {/* Chart canvas — Past/Present/Future zones are labeled in-chart via markArea, matching the Traffic Volume walk-forward chart */}
                  <div style={{ width: "100%", height: "460px" }}>
                    <PredictiveIncidentChart
                      dates={predictiveData.chartData.dates}
                      actualData={predictiveData.chartData.actualData}
                      predictedData={displayPredictedData}
                      ...
                      predictedColor={activeColor}
                      predictedLabel={`${activeModel?.model ?? predictiveData.summary.modelUsed} Prediction`}
                      forecastLabel={`${activeModel?.model ?? predictiveData.summary.modelUsed} Forecast`}
                    />
                  </div>

                  {activeModel && !activeModel.isChampion && (
                    <p style={{ fontSize: "0.76rem", color: "#94a3b8", margin: "10px 0 0" }}>
                      Note: Individual candidate metrics are exact; time-series curve for non-champion models is an illustrative approximation based on model MAE.
                    </p>
                  )}

                  {/* ── Real-World ML Validation Metrics — same table layout as the Traffic Volume chart's metrics table ── */}
                  <div style={{ background: "#f8fafc", borderRadius: "8px", padding: "16px", border: "1px solid #e2e8f0", marginTop: "18px" }}>
                    <h4 style={{ margin: "0 0 12px 0", fontSize: "0.95rem", color: "#334155", fontWeight: 600 }}>
                      Real-World ML Validation Metrics ({activeModel?.model ?? predictiveData.summary.modelUsed})
                    </h4>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem", minWidth: "520px" }}>
                        <thead>
                          <tr style={{ textAlign: "left", color: "#64748b", fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                            ...
                            <th style={{ padding: "6px 10px", fontWeight: 600, textAlign: "right" }}>Poisson Deviance</th>
                            <th style={{ padding: "6px 10px", fontWeight: 600, textAlign: "right" }}>R² Score</th>
                          </tr>
                        </thead>
                        <tbody>
                          {activeModel && (
                            <tr style={{ background: "#fff", borderTop: "1px solid #e2e8f0" }}>
                              <td style={{ padding: "10px", fontWeight: 700, color: "#0f172a" }}>
                                <span style={{ display: "inline-flex", alignItems: "center", gap: "8px" }}>
                                  <span style={{ width: 10, height: 10, borderRadius: "50%", background: activeColor }} />
                                  {activeModel.model}
                                  <span style={{ fontSize: "0.72rem", fontWeight: 500, color: activeModel.diagnosis === "OVERFITTING" ? "#b91c1c" : "#15803d" }}>
                                    ({modelStatusText(activeModel)})
                                  </span>
                                </span>
                              </td>
                              <td style={{ padding: "10px", textAlign: "right", color: "#0f172a" }}>{activeModel.RMSE.toFixed(4)}</td>
                              <td style={{ padding: "10px", textAlign: "right", color: "#0f172a" }}>{activeModel.MAE.toFixed(4)}</td>
                              <td style={{ padding: "10px", textAlign: "right", fontWeight: 700, color: activeColor }}>{activeModel.Poisson_Deviance.toFixed(4)}</td>
                              <td style={{ padding: "10px", textAlign: "right", fontWeight: 700, color: activeModel.diagnosis === "OVERFITTING" ? "#e11d48" : "#059669" }}>
                                {activeModel.R2.toFixed(4)}
                              </td>
                            </tr>
                          )}
                        </tbody>
                          ...
                          {(() => {
                            ...
                            allKnownModels.forEach(m => {
                              if (!displayModels.find(dm => dm.model === m)) {
                                displayModels.push({ model: m, MAE: null as any, RMSE: null as any, MASE: null as any, Poisson_Deviance: null as any, R2: null as any, Gap: null as any, diagnosis: "", rank: 99, isChampion: false });
                              }
                            });
