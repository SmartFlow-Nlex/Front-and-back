    ...
    <div className={shellClass}>
      ...
      <aside className="ds-sidebar">
        ...
        <nav className="ds-sidebar-nav">
          {visibleTabs.map((tab) => {
            // Sub-routes keep their parent tab lit — /dashboard/incident/hourly/
            // <date> is still the Incident section, and an exact match alone
            // would leave the whole sidebar dark while you're on a drill-down
            // page. "/dashboard" is excluded from the prefix test because it
            // prefixes every other route and would otherwise always match.
            const isActive =
              pathname === tab.href || (tab.href !== "/dashboard" && pathname.startsWith(`${tab.href}/`));
            return (
              <Link key={tab.href} href={tab.href} prefetch={true} className={`ds-sidebar-tab ${isActive ? "active" : ""}`}>
