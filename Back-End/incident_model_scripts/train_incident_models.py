...
def final_sarimax_forecast(feat: pd.DataFrame):
    ...
    combined_exog = pd.concat([val[CALENDAR_COLS].reset_index(drop=True), future_exog], ignore_index=True)
...
def build_all_final_predictions(feat: pd.DataFrame) -> tuple[dict[str, dict], dict[str, list]]:
    ...
    for name in MODEL_NAMES:
        try:
            ...
            set_all_seeds()
            val_pred, future_rows, imp = build_final_predictions(name, feat)
...
def write_to_db(conn, daily: pd.DataFrame, feat: pd.DataFrame, champion: str,
                 by_model: dict[str, dict], metadata: dict) -> None:
    ensure_schema(conn)
    ...
    def row_for(d, kind: str):
        ...
        return (d, kind, champ if champ is not None else 0.0, champion, num(yoy(d)), *preds)
...
def main() -> None:
    ...
    try:
        ...
        by_model, importances = build_all_final_predictions(feat)
        ...
        write_to_db(conn, daily, feat, champion, by_model, metadata)
        print("Committed.")
