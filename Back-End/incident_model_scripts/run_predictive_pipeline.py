...
counts, with lag/rolling/day-of-week features. Reads bronze.nlex_incidents
and writes ml_daily_actuals / ml_predictive_incidents / ml_training_metadata
(default/public schema, unqualified) in one transaction.
...
def main() -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            pred_rows = [
                (row.d.date(), "validation", float(p), CHAMPION_MODEL)
                for row, p in zip(val.itertuples(), val_pred)
            ] + [(d.date(), "future", p, CHAMPION_MODEL) for d, p in future_rows]
            psycopg2.extras.execute_values(
                cur,
                "INSERT INTO ml_predictive_incidents "
                "(forecast_date, prediction_type, predicted_incident_count, champion_model) VALUES %s",
                pred_rows,
            )
